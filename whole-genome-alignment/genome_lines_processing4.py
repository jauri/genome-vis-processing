...
# print "DEBUG!"
# for i in range(0, len(consensusOrder)):
#     print blocks[ consensusOrder[i] ]


# print "old conensus order" 
# print consensusOrder
...
while not done:
    ...
    #isGapBlock = [False]*len(neighborsBehindList) #could be a gap between each previous neighor

    for s in range(0, len(segs)):  #for all segs 
        # print "segment: %d" % segments[segs[s]["id"]]["id"]
        latestNeighbor = -1
        ...
        if latestNeighborPosition != b["compressedStart"]:
            # print " found a gap! my block is: %d and my id is %s and prev position is %d for neighbor %d" % (consensusOrder[i], sp, latestNeighborPosition, latestNeighbor["id"] )
            # print latestNeighbor["id"]
            # print "gap blocks"
            # print gapBlocks
            # print "latestNeighbor"
            # print latestNeighbor
            ...
            # print "looking for" 
            # print strNeigh
            if strNeigh in gapBlocks:
                # print "its here"
                whatever = 0
            else: 
                # print "making new block %d" % blockCount

                id = blockCount
                score = 0
                numberAligned = 0 # 1 for now
                newSegList = [] #empty for now
                maxLength = b["compressedStart"] - latestNeighborPosition #distance of the gap
                compressedNextTo = latestNeighbor["id"] #should be in front of latest neighbor
                compressedStart = latestNeighborPosition 
                ...
                #slot = len(slots)+2 #will resolve later NOT ADDED TO A SLOT LIST



                neighborsBehindList = []
                # print "adding "
                # print "%d" %latestNeighbor["id"]
                # gapBlocks.put( {"id": id, 'score': score, 'numberAligned': numberAligned, 'segList': newSegList, 'maxLength': maxLength,'compressedNextTo': compressedNextTo, "compressedStart": compressedStart, "neighborOrder": neighborOrder, "isGapBlock": isGapBlock, "neighborInFrontList": neighborFrontList, "slot": slot, "neighborsBehindList": [] } )
                ...
                consensusOrder.insert(i, id)
                i = i + 1  #skip  
                idx = i #move idx forward

                blocks.append(gapBlocks[ strNeigh])    
                b["neighborsBehindList"].append(id)
                # print "new nei behind list "
                # print b["neighborsBehindList"]
           

            gapBlock = gapBlocks[strNeigh]
            # gapSegs = gapBlocks[latestNeighbor["id"]]["segList"]
            ...
            gapBlock["segList"].append( {"id": newSeg["id"], "prevIdx": prevIdx, "prevSlot": prevSlot, "latestNeighborPosition": latestNeighborPosition, "latestNeighborBlock": latestNeighbor["id"], "class": "descending"} )
            ...
            gapBlocks[strNeigh] = gapBlock

            # for q in range(0, len(b["neighborsBehindList"]) ): #reset the blocks behind neighbors to include the gap
            #     if b["neighborsBehindList"][q] == latestNeighbor["id"]:
            #         b["neighborsBehindList"][q] = segId 
            ...
            # blocks.append(gapBlock)  ## DO THIS AT THE END...?
        #done creating a gap block 
    ...
    if i == len(consensusOrder):
        done = True

# pprint.pprint(gapBlocks)

# print "now classifying "

# print "new conensus order" 
# print consensusOrder
...
for i in range(2, len(consensusOrder)):
    ...
    for s in range(0, len(segs)):  #for all segs 
        ...
        # print "        %d " % blocks[ b["segList"][s]["latestNeighborBlock"] ]["id"]
